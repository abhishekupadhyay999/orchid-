...
const Offer = ({ pageTitle }) => {
  ...
  const fetchOfferData = async (offerID) => {
    try {
      ...
      if (offerData) {
        ...
        const newStatus = offerData.img_status === true ? "true" : "false" 
        setImgStatus(newStatus);
        console.log(offerData.img_status,newStatus)
  ...
  <div className="d-flex" style={{justifyContent:"space-between"}}>
    ...
    <button className="btn btn-primary" onClick={()=>router.push('/offer/offerlist')}>
                    Back
                  </button>
  </div>
</form>
