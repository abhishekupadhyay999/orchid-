...
const Element = ({pageTitle}) => {
    const router = useRouter()
      ...
      const fetch = async() =>{
        ...
        axios.post("http://localhost:4000/api/insta/get-instas",{},{
            headers:{
            }
        })
          .then((response) => {
            const formattedData = response.data.map((item) => ({
              id: item.id,
              title: item.img_text,
              image: item.img ? `data:image/png;base64,${Buffer.from(item.img.data).toString("base64")}` : null,
            }));
      ...
      const handleDelete = async (id) => {
        ...
        const response  = await axios.post('http://localhost:4000/api/insta/delete-insta',payload, {
            headers: { }
        });
        ...
        <div className="col-12">
          <div className="card">
            <div className="card-header" style={{ display:'flex', justifyContent:'space-between'}}>
              ...
              <Button variant="contained" color="success" onClick={()=> router.push('/insta/createInstaStories')}>
                Create
              </Button>
            </div>
